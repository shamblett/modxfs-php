...
PHP_FUSE_API int php_fuse_listxattr(const char * path, char * list, size_t list_len) {
	...
	if (Z_TYPE_P(arg_list) != IS_ARRAY) {
		...
		memcpy(list, Z_STRVAL_P(arg_list), list_len);
		zval_ptr_dtor(&arg_list);
	} else {
		HashPosition	pos;
		zval			**entry;
		...
		zend_hash_internal_pointer_reset_ex(Z_ARRVAL_P(arg_list), &pos);
		hash_key_type = zend_hash_get_current_key_ex(Z_ARRVAL_P(arg_list), &tmp_s_key, &tmp_s_key_len, &tmp_n_key, 0, &pos);	
		...
		if ( hash_key_type == HASH_KEY_IS_STRING )
		{
			...
			while (zend_hash_get_current_data_ex(Z_ARRVAL_P(arg_list), (void **)&entry, &pos) == SUCCESS) 
			{
				...
				if ((q-list) + (tmp_s_key_len-1) >= list_len) {
					break;
				}
				memcpy(q, tmp_s_key, tmp_s_key_len-1);
				q += tmp_s_key_len-1;
				...
				*q++ = '\n';
				zend_hash_move_forward_ex(Z_ARRVAL_P(arg_list), &pos);
				zend_hash_get_current_key_ex(Z_ARRVAL_P(arg_list), &tmp_s_key, &tmp_s_key_len, &tmp_n_key, 0, &pos);
			}
